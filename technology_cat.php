...
if (!$smarty->is_cached('technology_cat.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获得页面的内容 */
    ...
    $smarty->assign('artciles_list',    get_cat_articles($cat_id, $page, $size ,$keywords, 'technology_show'));
    $smarty->assign('cat_id',    $cat_id);
    /* 分页 */
    assign_pager('technology_cat', $cat_id, $count, $size, '', '', $page, $goon_keywords);

    $cat = article_cat_list(21, 0, false, 0);
    foreach ($cat as $key => $value) {
        if ($value['level'] !=1 || in_array($value['cat_id'], array(23)) ) {
            unset($cat[$key]);
        }
    }
...
$smarty->display('technology_cat.dwt', $cache_id);
