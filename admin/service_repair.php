...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许留言显示 */
        $sql = "UPDATE " .$ecs->table('service_repair'). " SET msg_status = 1 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
    ...
    {
        /* 禁止留言显示 */
        $sql = "UPDATE " .$ecs->table('service_repair'). " SET msg_status = 0 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act']=='view')
{
    ...
    $smarty->assign('action_link', array('text' => '维修申请列表', 'href'=>'service_repair.php?act=list_all'));
...
elseif ($_REQUEST['act']=='action')
{
    ...
    $sql = "UPDATE ".$ecs->table('service_repair')." SET reply='".$_POST['reply']."', reply_time = '".gmtime()."' WHERE msg_id = '".$_REQUEST['msg_id']."'";
    $db->query($sql);
...
function msg_list()
{
    ...
    $sql = "SELECT * " .
            "FROM " . $GLOBALS['ecs']->table('service_repair') . " AS f ".
            "WHERE 1=1 $where " .
...
function get_feedback_detail($id)
{
    ...
    $sql = "SELECT t.*, s.cat_name as guz, g.goods_name, g.goods_id ".
