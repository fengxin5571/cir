...
if ($action == 'default') {
	...
	$smarty->display('service_keep.dwt');
    ...
    {
        show_message('保养申请提交成功, 我们会尽快处理！', '继续提交', 'service_keep.php','info');
    }
    else
    {
        $err->show($_LANG['message_list_lnk'], 'service_keep.php');
    }
...
function get_cats()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('service') . ' WHERE cat_type = 2 ORDER BY sort_order ASC';
...
function add_message($message) {
    ...
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('service_keep') .
            " (msg_id, user_id, baoyang, goods_id, user_name, user_email, user_phone, user_company, msg_title, msg_type, msg_status,  msg_content, msg_time)".
            " VALUES (NULL, '$message[user_id]', '$message[baoyang]', '$message[goods_id]', '$message[user_name]', '$message[user_email]', ".
